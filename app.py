...
class BankPredictor:
    ...
    def predict_future(self, df, target_col, years_ahead=5):
        ...
        for i in range(years_ahead):
            ...
            # Safely calculate growth rate
            if len(last_values) >= 2:
                # Calculate percentage growth between last two values
                recent_growth = (last_values[-1] - last_values[-2]) / last_values[-2]
            else:
                recent_growth = 0.05  # Default 5% growth if not enough history
...
def main():
    try:
        ...
        analysis_type = st.sidebar.selectbox(
            "Select Analysis",
            ["📈 Deposits Analysis", "💰 Advances Analysis", "📊 Profitability Analysis", 
             "📋 Key Ratios Analysis", "🔮 Complete Forecast Report"]
        )
        ...
        if analysis_type == "📈 Deposits Analysis":
            ...
            # Combined growth
            fig.add_trace(go.Scatter(x=all_years, 
                                   y=[d+a for d,a in zip(all_deposits, all_advances)], 
                                   name='Total Business'), 
                         row=2, col=2)
